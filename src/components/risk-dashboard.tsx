  ...
  return (
    <section className="overflow-hidden apple-glass rounded-[2.25rem]">
      <div className="border-b border-black/[0.05] bg-white/35 px-6 py-5">
        <p className="apple-eyebrow">Safety & compliance center</p>
      ...
      <div className="grid gap-6 p-6 xl:grid-cols-[0.85fr_1.15fr]">
        <div className="grid gap-4">
          <div className="grid gap-3 sm:grid-cols-3">
            {[
              ...
              <div key={label} className="rounded-[1.75rem] border border-black/[0.07] bg-white/64 p-4">
                <p className="text-sm text-[#86868b]">{label}</p>
                <p className="mt-2 text-3xl font-bold text-[#1d1d1f]">{value}</p>
          ...
          <div className="rounded-[1.75rem] border border-black/[0.07] bg-white/64 p-5">
            <div className="flex items-center justify-between">
              <div>
                <p className="apple-eyebrow">Page rate limits</p>
                <h3 className="mt-2 text-xl font-semibold text-[#1d1d1f]">Giới hạn theo fanpage</h3>
              </div>
              <span className="rounded-full bg-[#1d1d1f] px-3 py-1 text-xs font-semibold text-white">
                {pages.length} page
              </span>
            </div>
            <div className="mt-4 grid gap-3">
              {pages.map((page) => (
                <div key={page.id} className="rounded-2xl bg-white px-4 py-3 text-sm text-[#6e6e73]">
          ...
          <div className="rounded-[1.75rem] border border-black/[0.07] bg-white/64 p-5">
            <div className="flex items-center justify-between">
              <div>
                <p className="apple-eyebrow">Risk signals</p>
                <h3 className="mt-2 text-xl font-semibold text-[#1d1d1f]">Tín hiệu rủi ro gần đây</h3>
              </div>
            ...
            <div className="mt-4 grid gap-3">
              ...
              {riskSignals.length === 0 && <p className="text-sm text-[#86868b]">Chưa có risk signal.</p>}
            </div>
          </div>
        </div>
        ...
        <div className="rounded-[1.75rem] border border-black/[0.07] bg-white/64 p-5">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <p className="apple-eyebrow">Approval workflow</p>
              <h3 className="mt-2 text-xl font-semibold text-[#1d1d1f]">Nội dung cần duyệt</h3>
            </div>
          ...
          <div className="mt-5 grid gap-4">
            {pendingApprovals.map((approval) => {
              ...
              return (
                <article key={approval.id} className="rounded-[1.75rem] border border-black/[0.07] bg-white p-5">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div className="max-w-2xl">
                      <p className="font-semibold text-[#1d1d1f]">{approval.post.title}</p>
                      <p className="mt-2 line-clamp-3 text-sm leading-7 text-[#6e6e73]">{approval.post.message}</p>
                    </div>
            ...
            {pendingApprovals.length === 0 && <p className="text-sm text-[#86868b]">Không có nội dung chờ duyệt.</p>}
            {result?.message && <p className="rounded-2xl bg-black/[0.04] px-4 py-3 text-sm text-[#424245]">{result.message}</p>}
          </div>
        </div>
      </div>
    </section>
